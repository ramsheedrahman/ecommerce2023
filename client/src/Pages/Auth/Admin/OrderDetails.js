...
function OrderDetails() {
  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        const { data } = await axios.get('http://localhost:8000/product/get-all-orders', {
          headers: {
  ...
  const orderStatushandler=async(value,id)=>{
    try {
      const { data } = await axios.put(`http://localhost:8000/product/order-status-update/${id}`,{status:value},{
        headers: {
          Authorization:User.token    },
      });
      console.log(data);
    ...
    <Layout>
      <div className="container-fluid">
        <div className="row" style={{marginTop:'100px'}}>
          ...
          {orders.length<1 ? (<h1 style={{color:'red'}}>Orders not Available</h1>):(
